...
our (%logins, $pidfile, $pid, @lffd, @lfino, @lfbuf, %db, %messengerports,
	 ...
	 @logignore, $loginterval, $ipscidr6, $cidr6, $gcidr6, %messengerips,
...
sub dochecks {
	...
	my ($reason, $ip, $app, $customtrigger, $customports, $customperm) = ConfigServer::RegexMain::processline ($line,$lgfile,\%globlogs);
		...
		foreach my $name (keys %blocklists) {
			...
			if ($getlist) {
				...
				if ($text =~ m[^PK\x03\x04] or $text =~ m[^PK\x05\x06] or $text =~ m[^PK\x07\x08]) {
					sysopen (BLOCK, "/var/lib/csf/csf.block.${name}.zip", O_WRONLY | O_CREAT) or &childcleanup(__LINE__,"*Error* Cannot open out file: $!");
					flock (BLOCK, LOCK_EX);
					print BLOCK $text;
					close (BLOCK);
					my @data;
...
sub logscanner {
	...
	unless ($childpid) {
		...
		unless (-z "/var/lib/csf/csf.logtemp" and $config{LOGSCANNER_EMPTY} == 0) {
			...
			sysopen (LOGTEMP,"/var/lib/csf/csf.logtemp", O_RDWR | O_CREAT);
			flock (LOGTEMP, LOCK_EX);
			my @data = <LOGTEMP>;
			seek (LOGTEMP, 0, 0);
			truncate (LOGTEMP, 0);
...
EOF
									...
									unless ($FORM{action} eq "tailcmd") {print "</div>\n"}
									print <<EOF;
...
sub faststart {
	...
	if (@faststart6nat) {
		if ($config{DEBUG} >= 1) {logfile("FASTSTART loading $text (IPv6 nat)")};
		my $status;
