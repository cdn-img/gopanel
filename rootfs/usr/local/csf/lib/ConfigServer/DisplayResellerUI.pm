	...
	if ($FORM{action} ne "" and !checkip(\$FORM{ip})) {
		...
		if ($FORM{action} eq "qallow" and $rprivs{$ENV{REMOTE_USER}}{ALLOW}) {
			if ($FORM{comment} eq "") {
				...
				ConfigServer::Logger::logfile("cPanel Reseller [$ENV{REMOTE_USER}]: ALLOW $FORM{ip}");
			}
			...
			if ($FORM{comment} eq "") {
				...
				ConfigServer::Logger::logfile("cPanel Reseller [$ENV{REMOTE_USER}]: DENY $FORM{ip}");
			}
			print "<p><form action='$script' method='post'><input type='hidden' name='mobi' value='$FORM{mobi}'><input type='submit' class='btn btn-default' value='Return'></form></p>\n";
		}
			...
			ConfigServer::Logger::logfile("cPanel Reseller [$ENV{REMOTE_USER}]: UNBLOCK $FORM{ip}");
		}
