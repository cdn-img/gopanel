...
sub report {
	...
	if (defined $ENV{WEBMIN_VAR} and defined $ENV{WEBMIN_CONFIG}) {
		...
		open (my $IN, "<", "/usr/local/directadmin/conf/directadmin.conf");
		flock ($IN, LOCK_SH);
		my @data = <$IN>;
		close ($IN);
		chomp @data;
	...
	foreach my $phpini (sort keys %phpinis) {
		...
		if (my @ls = grep {$_ =~ /^PHP License/} @conf) {
			...
			unless ($phpbin =~ m[^/opt/alt/php]) {
				&addline($status,"Check php version<br>[$phpbin]","Any version of PHP (Current: $version) older than v5.6.* is now obsolete and should be considered a security threat. You should upgrade exclusively to PHP v5.6+");
			}
			...
			&addline($status,"Check php for enable_dl or disabled dl()<br>[$phpbin]","You should modify $phpini and set:<br><b>enable_dl = Off</b><br>This prevents users from loading php modules that affect everyone on the server. Note that if use dynamic libraries, such as ioncube, you will have to load them directly in the PHP configuration");
			
			$status = 1;
			if (my @ls = grep {$_ =~ /^disable_functions\s*=>.*\,/i} @conf) {
				$status = 0;
			}
			&addline($status,"Check php for disable_functions<br>[$phpbin]","You should modify the PHP configuration and disable commonly abused php functions, e.g.:<br><b>disable_functions = show_source, system, shell_exec, passthru, exec, phpinfo, popen, proc_open</b><br>Some client web scripts may break with some of these functions disabled, so you may have to remove them from this list");

			$status = 1;
			if (my @ls = grep {$_ =~ /^disable_functions\s*=>.*ini_set.*/i} @conf) {
				$status = 0;
			}
			&addline($status,"Check php for ini_set disabled<br>[$phpbin]","You should consider adding ini_set to the disable_functions in the PHP configuration as this setting allows PHP scripts to override global security and performance settings for PHP scripts. Adding ini_set can break PHP scripts and commenting out any use of ini_set in such scripts is advised");
			...
			if ($oldver < 5.4) {
				...
				&addline($status,"Check php for register_globals<br>[$phpbin]","You should modify the PHP configuration and set:<br><b>register_globals = Off</b><br>unless it is absolutely necessary as it is seen as a significant security risk");
...
sub apachecheck {
	...
	unless ($config{DIRECTADMIN}) {
		...
		&addline($status,"Check apache for mod_cloudflare","This module logs the real users IP address to Apache. If this is reported to lfd via ModSecurity, cxs or some other vector through Apache it will lead to that IP being blocked, but because the IP is coming through the CloudFlare service the IP will <b>not</b> be blocked as so far as iptables is concerned the originating IP address is CloudFlare itself and the abuse will continue.");
